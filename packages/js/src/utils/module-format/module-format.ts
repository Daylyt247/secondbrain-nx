...
export function getTsConfigModuleFormat(
  ...
  if (m === ts.ModuleKind.NodeNext || m === ts.ModuleKind.Node16) return null;
  if (
    m === ts.ModuleKind.ES2015 ||
    m === ts.ModuleKind.ES2020 ||
    m === ts.ModuleKind.ES2022 ||
    m === ts.ModuleKind.ESNext
  ) {
    return 'esm';
  }
  return 'cjs';
